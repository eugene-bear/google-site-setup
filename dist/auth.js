...
export function loadServiceAccountKey() {
    ...
    try {
        ...
        throw new Error(`Cannot read service account key file at: ${keyPath}\n` +
    ...
    try {
        ...
        throw new Error(`Service account key file is not valid JSON: ${keyPath}`);
    }
    if (!key.client_email || !key.private_key) {
        throw new Error(`Service account key file is missing required fields (client_email, private_key): ${keyPath}`);
    }
    cachedKey = key;
    return key;
}
