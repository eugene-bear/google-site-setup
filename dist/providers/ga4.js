...
export async function assertGA4AccountAccess(accountId, serviceAccountEmail) {
    const admin = getAnalyticsAdminClient();
    try {
        const res = await admin.accounts.list({});
        const accounts = res.data.accounts || [];
        const match = accounts.some((a) => a.name?.replace("accounts/", "") === accountId);
        if (!match) {
            const t = translateGoogleError(new Error(`GA4 account ${accountId} not in accounts.list()`), {
                service: "ga4",
...
export async function provisionGA4(options) {
...
async function findExistingProperty(admin, parent, domain) {
    try {
