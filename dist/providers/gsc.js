...
export function isSupportedVerificationMethod(v) {
    return SUPPORTED_METHODS.includes(v);
}
export function listSupportedVerificationMethods() {
    return [...SUPPORTED_METHODS];
}
export async function provisionGSC(options) {
    const method = options.verificationMethod || "meta";
    if (!isSupportedVerificationMethod(method)) {
        throw new Error(`Unsupported verification method: ${method}. Supported: ${SUPPORTED_METHODS.join(", ")}`);
    }
    ...
    if (options.sitemapUrl) {
        if (verified) {
            ...
            console.log(`  Sitemap submission deferred — verify the site first, then run "google-site-setup status ${options.domain} --fix"`);
...
async function submitSitemap(searchconsole, siteUrl, sitemapUrl, domain) {
    ...
    try {
        ...
        const code = err instanceof Error && "code" in err
            ...
            : undefined;
        const msg = err instanceof Error ? err.message : String(err);
        const isUnverified = code === 403 || /sufficient permission|forbidden|user does not have/i.test(msg);
        if (isUnverified) {
            console.log(`  Sitemap deferred — site ${domain} isn't verified yet. ` +
...
async function checkExistingSite(searchconsole, siteUrl) {
    try {
...
function renderInstructions(method, domain, token) {
    const lines = [];
