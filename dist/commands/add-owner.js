...
export async function addOwnerCommand(domain, email) {
    ...
    try {
        ...
        const site = list.data.items?.find((item) => item.site?.type === "INET_DOMAIN" &&
            item.site?.identifier === cleanDomain);
        if (!site) {
            console.error(`  Error: ${cleanDomain} is not verified yet.\n` +
