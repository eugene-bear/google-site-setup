...
export async function addEventCommand(domain, eventName, opts) {
    ...
    if (!siteConfig?.ga4?.measurementId) {
        console.error(`Error: No GA4 measurement ID found for ${cleanDomain}.\n` +
            `Run "google-site-setup provision --domain ${cleanDomain}" first.`);
        process.exit(1);
        return;
    }
    ...
    if (!triggerId)
        throw new Error("Trigger creation returned no triggerId");
    ...
    if (createdVariables.length > 0) {
        tagParams.push({
            type: "list",
            key: "eventParameters",
            list: createdVariables.map((param) => ({
                type: "map",
                map: [
                    { type: "template", key: "name", value: param },
