...
import { loadServiceAccountKey, getAnalyticsAdminClient, getTagManagerClient, getSearchConsoleClient, } from "../auth.js";
...
export async function initCommand() {
    ...
    if (existing.serviceAccountEmail) {
        ...
        if (existing.gaAccountId)
            console.log(`  GA4 account:     ${existing.gaAccountId}`);
        if (existing.gtmAccountId)
            console.log(`  GTM account:     ${existing.gtmAccountId}`);
        console.log("");
    ...
    const tzChoice = await ask(rl, `  Pick a default timezone (1-${TZ_CHOICES.length}, or press Enter for UTC, or type a custom IANA name): `);
    ...
    const ccyChoice = await ask(rl, `  Pick a default currency (1-${CCY_CHOICES.length}, or press Enter for USD, or type an ISO code): `);
