...
import { getAnalyticsAdminClient, getTagManagerClient, getSearchConsoleClient, getSiteVerificationClient, } from "../auth.js";
const API_METHOD = {
    meta: "META",
    file: "FILE",
    dns: "DNS_TXT",
...
export async function statusCommand(domain, opts) {
    if (!domain) {
    ...
    if (status.gtm.containerPublicId && globalConfig.gtmAccountId) {
        try {
            ...
            const match = res.data.container?.find((c) => c.publicId === status.gtm.containerPublicId);
            if (match) {
    ...
    const siteUrl = SITE_TYPE[method] === "INET_DOMAIN"
        ? `sc-domain:${domain}`
        : `https://${domain}/`;
    try {
    ...
    if (opts.fix) {
        ...
        if (status.gsc.live && !status.gsc.verified) {
            ...
            try {
                ...
                await siteVerification.webResource.insert({
                    ...
                    requestBody: {
                        site: {
                            ...
                            identifier: SITE_TYPE[method] === "INET_DOMAIN"
                                ? domain
                                : `https://${domain}/`,
                        },
                    },
                ...
                console.log(`  ✗ Verification failed — the ${method} record may not be live yet: ${msg}`);
            }
        }
        // Submit pending sitemap (may be from a deferred earlier provision)
        const sitemapToSubmit = status.gsc.sitemapPending ||
    ...
    console.log(`  ${"GA4 Analytics".padEnd(20)} ${yn(status.ga4.configured).padEnd(14)} ${yn(status.ga4.live).padEnd(8)} ${status.ga4.measurementId || "—"}`);
    console.log(`  ${"Tag Manager".padEnd(20)} ${yn(status.gtm.configured).padEnd(14)} ${yn(status.gtm.live).padEnd(8)} ${status.gtm.containerPublicId || "—"}${status.gtm.published ? " (published)" : ""}`);
    const gscDetails = `verified: ${yn(status.gsc.verified)}, sitemap: ` +
        (status.gsc.sitemapSubmitted
            ? "yes"
            : status.gsc.sitemapDeferred
                ? `deferred (${status.gsc.sitemapPending})`
                : "no");
    console.log(`  ${"Search Console".padEnd(20)} ${yn(status.gsc.configured).padEnd(14)} ${yn(status.gsc.live).padEnd(8)} ${gscDetails}`);
