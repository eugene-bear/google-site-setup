...
function loadPromptState() {
    if (!existsSync(PROMPT_FLAG_PATH))
        return {};
    try {
...
export function shouldAutoPrompt() {
    ...
    if (!cmd || cmd === "install-skill" || cmd === "uninstall-skill")
        return false;
    if (cmd === "--help" || cmd === "-h" || cmd === "--version" || cmd === "-V")
        return false;
    return true;
}
export async function autoPromptInstallSkill() {
    if (!shouldAutoPrompt())
        return;
    const rl = readline.createInterface({ input: process.stdin, output: process.stdout });
    const answer = await new Promise((resolve) => rl.question("Claude Code skill for google-site-setup is not installed.\nInstall it now to ~/.claude/skills/google-site-setup? [Y/n] ", resolve));
    rl.close();
