...
} from "../templates/conversion-tags.js";

interface SetupConversionsOptions {
  platforms?: string;
  json?: boolean;
}
...
export async function setupConversionsCommand(
  domain: string,
  opts: SetupConversionsOptions
) {
  ...
  // Parse platforms
  let platforms: ConversionPlatform[] = ALL_PLATFORMS;
  if (opts.platforms) {
    ...
    const invalid = requested.filter(
      (p) => !ALL_PLATFORMS.includes(p as ConversionPlatform)
    );
    if (invalid.length > 0) {
      console.error(
    ...
    platforms = requested as ConversionPlatform[];
  }
  ...
  try {
    ...
    existingTagNames = new Set(
      (listed.data.tag || []).map((t) => t.name || "")
    );
  } catch {
    /* if listing fails, fall through; create may still error individually */
  }

  const created: ConversionPlatform[] = [];
  const skipped: ConversionPlatform[] = [];
  ...
  if (versionId) console.log(`    Version: ${versionId} (published)`);
  if (created.length > 0) {
    console.log("\n  Next steps:");
