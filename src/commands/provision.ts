...
interface TranslatedFailure {
  summary: string;
  fix: string;
}

function translatedFromError(
  err: unknown,
  service: "ga4" | "gtm" | "gsc",
  operation: string,
  domain?: string
): TranslatedFailure {
  const pre = (err as { translated?: TranslatedFailure }).translated;
  if (pre) return pre;
  const t = translateGoogleError(err, { service, operation, domain });
  return { summary: t.summary, fix: t.fix };
}

export async function provisionCommand(opts: ProvisionOptions) {
  ...
  const timezone =
    opts.timezone || globalConfig.defaultTimezone || FALLBACK_TIMEZONE;
  const currency =
    opts.currency || globalConfig.defaultCurrency || FALLBACK_CURRENCY;

  // Track sources for JSON output
  const accountIdSource: ProvisionResult["accountIdSource"] = {
  ...
  let verificationMethod: VerificationMethod = "meta";
  if (opts.verificationMethod) {
    if (!isSupportedVerificationMethod(opts.verificationMethod)) {
      console.error(
        `Error: --verification-method must be one of ${listSupportedVerificationMethods().join(", ")}`
      );
      process.exit(1);
      return;
    }
  ...
  if (!opts.skipGa4 && !gaAccount) {
    console.error(
      'Error: --ga-account is required (or run "google-site-setup init" to save defaults; or pass --skip-ga4)'
    );
    process.exit(1);
    return;
  }
  if (!opts.skipGtm && !gtmAccount) {
    console.error(
      'Error: --gtm-account is required (or run "google-site-setup init" to save defaults; or pass --skip-gtm)'
    );
    process.exit(1);
    return;
  }
  ...
  let savedMeasurementId =
    opts.measurementId || existingSite?.ga4?.measurementId;

  if (opts.skipGa4 && !opts.skipGtm && !savedMeasurementId) {
    console.error(
      "Error: --skip-ga4 requires either an existing GA4 measurement ID for this domain " +
  ...
  if (!opts.dryRun) {
    ...
    console.log(
      `  GA account:  ${gaAccount || "(skipped)"}${
        accountIdSource?.ga4 === "config" ? " (from saved config)" : accountIdSource?.ga4 === "flag" ? " (from flag)" : ""
      }`
    );
    console.log(
      `  GTM account: ${gtmAccount || "(skipped)"}${
        accountIdSource?.gtm === "config" ? " (from saved config)" : accountIdSource?.gtm === "flag" ? " (from flag)" : ""
      }`
    );
    console.log(`  Timezone:    ${timezone}`);
    console.log(`  Currency:    ${currency}`);
    console.log(`  Verification: ${verificationMethod}`);
    if (opts.skipGa4) console.log(`  Skipping:    GA4`);
    if (opts.skipGtm) console.log(`  Skipping:    GTM`);
    if (opts.skipGsc) console.log(`  Skipping:    GSC`);
    if (usingSavedConfig && !opts.confirmSavedConfig && !process.stdout.isTTY) {
      console.error(
        "  Warning: account IDs came from saved config but --confirm-saved-config was not passed. " +
  ...
  if (opts.dryRun) {
    ...
    if (existingSite) {
      ...
      if (existingSite.ga4) console.log(`  GA4: ${existingSite.ga4.measurementId}`);
      if (existingSite.gtm) console.log(`  GTM: ${existingSite.gtm.containerPublicId}`);
      if (existingSite.gsc) console.log(`  GSC: ${existingSite.gsc.verified ? "verified" : "unverified"}`);
    } else {
  ...
  const siteConfig: SiteConfig = existingSite || {
    domain,
  ...
  if (opts.skipGsc) {
    ...
    if (result.gsc.sitemapDeferred) {
      ...
      console.log(
        `    Sitemap:         ${result.gsc.sitemapSubmitted ? "submitted" : opts.sitemap ? "failed" : "not provided"}`
      );
    }
