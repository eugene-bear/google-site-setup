...
export async function assertGTMAccountAccess(
  accountId: string,
  serviceAccountEmail?: string
): Promise<void> {
  ...
  try {
    ...
    if (!match) {
      const t = translateGoogleError(
        new Error(`GTM account ${accountId} not in accounts.list()`),
        {
          service: "gtm",
...
export async function provisionGTM(options: GTMOptions): Promise<GTMResult> {
...
async function findExistingContainer(
  ...
  accountPath: string,
  domain: string
): Promise<Omit<GTMResult, "skipped"> | null> {
  try {
